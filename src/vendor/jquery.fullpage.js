...
c.afterRender.call(this)
            ...
            if (l[a]) {
                if (a ==
'down') var c = 'bottom', f = b.fn.fullpage.moveSectionDown; else c = 'top', f = b.fn.fullpage.moveSectionUp; if (d.length > 0) if (c = c === 'top' ? !d.scrollTop() : c === 'bottom' ? d.scrollTop() + 1 + d.innerHeight() >= d[0].scrollHeight : void 0, c)f(); else return !0; else f()
            }
        } function sa (a) {
...
a.wheelDelta || -a.deltaY || -a.detail)); c.scrollBar && (a.preventDefault ? a.preventDefault() : a.returnValue = !1); a = b('.fp-section.active'); a = V(a); m || (d < 0 ? z('down', a) : z('up', a)); return !1
            }
        } function Y (a) { var d = b('.fp-section.active').find('.fp-slides'); if (d.length && !t) { var e = d.find('.fp-slide.active'), f = null, f = a === 'prev' ? e.prev('.fp-slide') : e.next('.fp-slide'); if (!f.length) { if (!c.loopHorizontal) return; f = a === 'prev' ? e.siblings(':last') : e.siblings(':first') }t = !0; w(d, f) } } function Z () { b('.fp-slide.active').each(function () { J(b(this)) }) }
        function q (a, d, e) {
            var f = a.position(); if (typeof f !== 'undefined' && (d = {element: a, callback: d, isMovementUp: e, dest: f, dtop: f.top, yMovement: G(a), anchorLink: a.data('anchor'), sectionIndex: a.index('.fp-section'), activeSlide: a.find('.fp-slide.active'), activeSection: b('.fp-section.active'), leavingSection: b('.fp-section.active').index('.fp-section') + 1, localIsResizing: x}, !(d.activeSection.is(a) && !x || c.scrollBar && b(window).scrollTop() === d.dtop))) {
                if (d.activeSlide.length) var g = d.activeSlide.data('anchor'), h = d.activeSlide.index()
                c.autoScrolling && c.continuousVertical && typeof d.isMovementUp !== 'undefined' && (!d.isMovementUp && d.yMovement == 'up' || d.isMovementUp && d.yMovement == 'down') && (d.isMovementUp ? b('.fp-section.active').before(d.activeSection.nextAll('.fp-section')) : b('.fp-section.active').after(d.activeSection.prevAll('.fp-section').get().reverse()), n(b('.fp-section.active').position().top), Z(), d.wrapAroundElements = d.activeSection, d.dest = d.element.position(), d.dtop = d.dest.top, d.yMovement = G(d.element)); a.addClass('active').siblings().removeClass('active')
...
!1; b.isFunction(a.callback) && a.callback.call(this)
        ...
        } function da () { if (!F) { var a = window.location.hash.replace('#', '').split('/'), b = a[0], a = a[1]; if (b.length) { var c = typeof p === 'undefined', f = typeof p === 'undefined' && typeof a === 'undefined' && !t; (b && b !== p && !c || f || !t && K != a) && L(b, a) } } } function w (a, d) {
            var e = d.position(), f = a.find('.fp-slidesContainer').parent(), g = d.index(), h = a.closest('.fp-section'), k = h.index('.fp-section'), l = h.data('anchor'), n = h.find('.fp-slidesNav'), m = d.data('anchor'), q = x; if (c.onSlideLeave) {
                var p =
h.find('.fp-slide.active').index(), r; r = p == g ? 'none' : p > g ? 'left' : 'right'; q || r === 'none' || b.isFunction(c.onSlideLeave) && c.onSlideLeave.call(this, l, k + 1, p, r)
            }d.addClass('active').siblings().removeClass('active'); typeof m === 'undefined' && (m = g); !c.loopHorizontal && c.controlArrows && (h.find('.fp-controlArrow.fp-prev').toggle(g != 0), h.find('.fp-controlArrow.fp-next').toggle(!d.is(':last-child'))); h.hasClass('active') && I(g, m, l, k); var u = function () {
                q || b.isFunction(c.afterSlideLoad) && c.afterSlideLoad.call(this, l, k +
1, m, g); t = !1
            }; c.css3 ? (e = 'translate3d(-' + e.left + 'px, 0px, 0px)', ea(a.find('.fp-slidesContainer'), c.scrollingSpeed > 0).css(fa(e)), setTimeout(function () { u() }, c.scrollingSpeed, c.easing)) : f.animate({scrollLeft: e.left}, c.scrollingSpeed, c.easing, function () { u() }); n.find('.active').removeClass('active'); n.find('li').eq(g).find('a').addClass('active')
        } function ga () {
            ha(); if (C) {
                if (b(document.activeElement).attr('type') !== 'text') {
...
M = a)
            ...
            var b = 'all ' + c.scrollingSpeed + 'ms ' + c.easingcss3; a.removeClass('fp-notransition'); return a.css({'-webkit-transition': b,
                transition: b})
        } function O (a) { return a.addClass('fp-notransition') } function xa (a, d) { if (a < 825 || d < 900) { var c = Math.min(100 * a / 825, 100 * d / 900).toFixed(2); b('body').css('font-size', c + '%') } else b('body').css('font-size', '100%') } function H (a, d) { c.menu && (b(c.menu).find('.active').removeClass('active'), b(c.menu).find('[data-menuanchor="' + a + '"]').addClass('active')); c.navigation && (b('#fp-nav').find('.active').removeClass('active'), a ? b('#fp-nav').find('a[href="#' + a + '"]').addClass('active') : b('#fp-nav').find('li').eq(d).find('a').addClass('active')) }
...
h; if (c.paddingTop || c.paddingBottom)b = a, b.hasClass('fp-section') || (b = a.closest('.fp-section')), a = parseInt(b.css('padding-top')) + parseInt(b.css('padding-bottom')), b = h - a; return b
        } function aa (a, b) { b ? ea(g) : O(g); g.css(fa(a)); setTimeout(function () { g.removeClass('fp-notransition') }, 10) } function L (a, d) { typeof d === 'undefined' && (d = 0); var c = isNaN(a) ? b('[data-anchor="' + a + '"]') : b('.fp-section').eq(a - 1); a === p || c.hasClass('active') ? ma(c, d) : q(c, function () { ma(c, d) }) } function ma (a, b) {
...
a.find('.fp-slides'), f = c.find('[data-anchor="' + b + '"]'); f.length || (f = c.find('.fp-slide').eq(b)); f.length && w(c, f)
            ...
            var g = ''; c.anchors.length ? (a ? (typeof e !== 'undefined' && (g = e), typeof b ===
'undefined' && (b = a), K = b, na(g + '/' + b)) : (typeof a !== 'undefined' && (K = b), na(e)), D(location.hash)) : typeof a !== 'undefined' ? D(f + '-' + a) : D(String(f))
...
document.createElement('p'), b, c = {webkitTransform: '-webkit-transform', OTransform: '-o-transform', msTransform: '-ms-transform', MozTransform: '-moz-transform', transform: 'transform'}; document.body.insertBefore(a, null); for (var f in c) void 0 !== a.style[f] && (a.style[f] = 'translate3d(1px,1px,1px)', b = window.getComputedStyle(a).getPropertyValue(c[f])); document.body.removeChild(a); return void 0 !== b && b.length > 0 && b !== 'none'
        ...
        } function Q (a, b, e) { c[a] = b; e !== 'internal' && (N[a] = b) } function E (a, b) { console && console[a] && console[a]('fullPage: ' + b) }c = b.extend({menu: !1,
            anchors: [],
            navigation: !1,
...
{easeInQuart: function (a, b, c, f, g) { return f * (b /= g) * b * b * b + c }}); b.fn.fullpage.setAutoScrolling = function (a, d) {
    Q('autoScrolling', a, d); var e = b('.fp-section.active'); c.autoScrolling && !c.scrollBar ? (b('html, body').css({overflow: 'hidden', height: '100%'}), b.fn.fullpage.setRecordHistory(c.recordHistory, 'internal'), g.css({'-ms-touch-action': 'none', 'touch-action': 'none'}), e.length && n(e.position().top)) : (b('html, body').css({overflow: 'visible', height: 'initial'}), b.fn.fullpage.setRecordHistory(!1, 'internal'), g.css({'-ms-touch-action': '',
        'touch-action': ''}), n(0), b('html, body').scrollTop(e.position().top))
...
r, !1)) : document.detachEvent('onmousewheel', r)
    ...
    if (typeof c !== 'undefined')c = c.replace(' ', '').split(','), b.each(c, function (c, d) { switch (d) { case 'up':l.up = a; break; case 'down':l.down = a; break; case 'left':l.left = a; break; case 'right':l.right = a; break; case 'all':b.fn.fullpage.setAllowScrolling(a) } }); else if (a) {
        if (b.fn.fullpage.setMouseWheelScrolling(!0), C || P) {
            MSPointer = oa(), b(document).off('touchstart ' + MSPointer.down).on('touchstart ' + MSPointer.down, ta), b(document).off('touchmove ' +
...
}; b.fn.fullpage.moveTo = function (a, c) { var e = '', e = isNaN(a) ? b('[data-anchor="' + a + '"]') : b('.fp-section').eq(a - 1); typeof c !== 'undefined' ? L(a, c) : e.length > 0 && q(e) }; b.fn.fullpage.moveSlideRight = function () { Y('next') }; b.fn.fullpage.moveSlideLeft = function () { Y('prev') }; b.fn.fullpage.reBuild = function (a) {
...
}; var t = !1, C = navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry|BB10|Windows Phone|Tizen|Bada)/), P = 'ontouchstart' in window || navigator.msMaxTouchPoints > 0 || navigator.maxTouchPoints, g = b(this), h = b(window).height(), m = !1, x = !1, p, K, k, ca = 'fullpage-wrapper', l = {up: !0, down: !0, left: !0, right: !0}, N = jQuery.extend(!0, {}, c); b.fn.fullpage.setAllowScrolling(!0); c.css3 && (c.css3 = za()); b(this).length ? (g.css({height: '100%',
    position: 'relative'}), g.addClass(ca)) : E('error', "Error! Fullpage.js needs to be initialized with a selector. For example: $('#myContainer').fullpage();"); b(c.sectionSelector).each(function () { b(this).addClass('fp-section') }); b(c.slideSelector).each(function () { b(this).addClass('fp-slide') }); c.navigation && qa(); b('.fp-section').each(function (a) {
        var d = b(this), e = b(this).find('.fp-slide'), f = e.length; a || b('.fp-section.active').length !== 0 || b(this).addClass('active'); b(this).css('height', h + 'px'); (c.paddingTop ||
c.paddingBottom) && b(this).css('padding', c.paddingTop + ' 0 ' + c.paddingBottom + ' 0'); typeof c.sectionsColor[a] !== 'undefined' && b(this).css('background-color', c.sectionsColor[a]); typeof c.anchors[a] !== 'undefined' && b(this).attr('data-anchor', c.anchors[a]); if (f > 1) {
...
g + '%'); c.verticalCentered && ka(b(this))
    }); d = d.find('.fp-slide.active'); d.length == 0 ? e.eq(0).addClass('active') : J(d)
} else c.verticalCentered && ka(b(this))
    }).promise().done(function () {
        b.fn.fullpage.setAutoScrolling(c.autoScrolling, 'internal'); var a = b('.fp-section.active').find('.fp-slide.active'); a.length && (b('.fp-section.active').index('.fp-section') != 0 || b('.fp-section.active').index('.fp-section') == 0 && a.index() != 0) && J(a); c.fixedElements && c.css3 && b(c.fixedElements).appendTo('body'); c.navigation && (k.css('margin-top',
'-' + k.height() / 2 + 'px'), k.find('li').eq(b('.fp-section.active').index('.fp-section')).find('a').addClass('active')); c.menu && c.css3 && b(c.menu).closest('.fullpage-wrapper').length && b(c.menu).appendTo('body'); c.scrollOverflow ? (document.readyState === 'complete' && R(), b(window).on('load', R)) : b.isFunction(c.afterRender) && c.afterRender.call(this); ha(); a = window.location.hash.replace('#', '').split('/')[0]; if (a.length) {
...
: (n(0), D(a), b('html, body').scrollTop(d.position().top)), H(a, null), b.isFunction(c.afterLoad) && c.afterLoad.call(this, a, d.index('.fp-section') + 1), d.addClass('active').siblings().removeClass('active'))
        ...
        if (c.keyboardScrolling && c.autoScrolling && (a.which != 40 && a.which != 38 || a.preventDefault(),
